...
// Remove lessons from user's completed list
export async function POST(request: Request) {
  try {
    ...
    const updatedLessons = currentLessons.filter((id: string) => !lessonIds.includes(id))
    ...
    return NextResponse.json({ 
      success: true, 
      removed: lessonIds.length,
      total: updatedLessons.length
    })
  } catch (error) {
    console.error('Error removing lessons:', error)
    return NextResponse.json({ error: 'Failed to remove lessons' }, { status: 500 })
  }
}
